...
def scan_files():
    scanned = [] 
     for root, _, files in 
os.walk(WATCH_DIR): 
        for file in files: if any(file.endswith(ext) for
 ext in SCAN_TYPES):
                full_path =
...
def save_memory_wheel(data): 
    ...
    filename =
 f"wheel_{int(time.time())}.json" 
        full_path = os.path.join(MEMORY_DIR, 
filename) 
  with open(full_path, 'w') as f:
       json.dump(data, f, indent=2) 
print(f"[+] Saved memory wheel Saved to
 {full_path}")

def run_cycle(): 
  print(f"[+] Rasta Dave is traversing 
the system...") 
