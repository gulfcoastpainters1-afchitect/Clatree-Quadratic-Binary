...
# and keeps the rhythm of observation alive.Trinary_dave.py

def trinary_interpreter(cycle_count):
    """Map the current count into binary/trinary values."""
    binary_state = cycle_count % 2
    trinary_state = cycle_count % 3

    if trinary_state == 0:
        phrase = "Expansion phase — open and observing."
    elif trinary_state == 1:
        phrase = "Equilibrium phase — balanced reasoning."
    else:
        phrase = "Compression phase — condensing insight."

    return binary_state, trinary_state, phrase
