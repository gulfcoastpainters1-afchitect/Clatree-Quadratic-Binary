...
class Message(BaseModel): user: str text: str
...
def crawl_files(root="/",
         keywords=["ruby", "ai", "daemon", "runtime"]): matched = [] 
     for dirpath, _, filenames in os.walk(root): 
        for file in filenames: if any(k in file.lower() for k in keywords): 
   full_path = os.path.join(dirpath, file) matched.append(full_path) 
return matched

#persistent memory

def save_memory():
 with open(state_file, "w") as f: json.dump(memory_log, f, indent=2)

   def load_memory():
      global memory_log try: with open(state_file) 
  as f: memory_log = json.load(f) 
except FileNotFoundError: memory_log = []
 
#chat endpoint

@app.post("/chat") 
 async def chat(request: Message): 
     ts = datetime.utcnow().isoformat() memory_entry = {"user": request.user,
        "message": request.text, "timestamp": ts} 
  memory_log.append(memory_entry),save_memory()
...
@app.post("/state/{name}/{value}") 
 def update_state(name: str, value: float): spo.update_state(name, value) 
